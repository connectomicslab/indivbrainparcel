...
def _build_args_parser():
    ...
    p = argparse.ArgumentParser(
        formatter_class=SmartFormatter, description="\n Help \n"
    )
    ...
    requiredNamed.add_argument(
        ...
        " If the ouput folder is not supplied the results will be stored in a "
        "derivatives"
        " folder next to the freesurfer subjects directory. \n",
        default=None,
    )
...
def _launch_annot2ind(fs_annot, ind_annot, hemi, out_dir, fullid, atlas):
    ...
    fs_subj_temp = CorticalParcellation(
        subjects_dir=fssubj_dir,
        subjid=fullid,
        hemi=hemi,
        annot=ind_annot,
        surface="white",
    ...
    out_annot = os.path.join(
        out_dir, fullid + "_hemi-" + hemicad + "_space-orig_" + atlas + "_dparc.annot"
    )
    subprocess.run(
        ["cp", ind_annot, out_annot], stdout=subprocess.PIPE, universal_newlines=True
    )
...
def _launch_freesurfer(t1file: str, fssubj_dir: str, fullid: str):
    ...
    subprocess.run(
        ["recon-all", "-subjid", "-i", t1file, fullid, "-all"],
        stdout=subprocess.PIPE,
        universal_newlines=True,
    )
...
def _launch_surf2vol(fssubj_dir, out_dir, fullid, atlas, gm_grow):
    ...
    for g in gm_grow:
        out_vol = os.path.join(
            out_dir, fullid + "_space-orig_" + atlas_str + "grow" + g + "mm_dseg.nii.gz"
        )
...
def _compute_abased_thal_parc(t1, vol_tparc, deriv_dir, subjid, aseg_nii, out_str):
    ...
    thal_spam = os.path.join(
        data_dir, "thalamic_nuclei_MIALatlas", "Thalamus_Nuclei-HCP-4DSPAMs.nii.gz"
    )
    t1_temp = os.path.join(
        data_dir,
    ...
    mial_thalparc = os.path.join(
        mial_dir, subjid + "_space-orig_desc-" + out_str + "_dseg.nii.gz"
    )
    mial_thalspam = os.path.join(
        mial_dir, subjid + "_space-orig_desc-" + out_str + "_probseg.nii.gz"
    )
    ...
    subprocess.run(
        [
            ...
            t1,
            "-t",
            defFile + "desc-t12mni_1InverseWarp.nii.gz",
            "-t",
            "[" + defFile + "desc-t12mni_0GenericAffine.mat,1]",
            "-n",
            "Linear",
        ],
        stdout=subprocess.PIPE,
...
def _spams2maxprob(
    ...
    thalm_names = [
        ...
        "ventral-anterior",
        "mediodorsal",
        "lateral-posterior-ventral-posterior-group",
        "pulvinar-medial-centrolateral-group",
        "ventrolateral",
        "ventral-posterior-ventrolateral-group",
    ]
    prefix = "thal-lh-"
    ...
    imgcoll = nib.Nifti1Image(
        maxprob_thr.astype("int16") + maxprob_thl.astype("int16"), affine
    )
    nib.save(imgcoll, maxpName)

    # Creating the corresponding TSV file
    _parc_tsv_table(
        np.concatenate((thalm_codesl, thalm_codesr)),
    ...
    luttable = [
        "# $Id: <BIDsDirectory>/derivatives/{} {} \n".format(
            "/".join(lutlist[-5:]), date_time
        ),
    ...
    luttable.append(
        "# Left Hemisphere. Thalamic nuclei parcellation (MIAL, Najdenovska and Alemán-Gómez et al, 2018)"
    )
    for roi_pos, roi_name in enumerate(thalm_namesl):
        luttable.append(
            "{:<4} {:<50} {:>3} {:>3} {:>3} {:>3}".format(
                roi_pos + 1,
                roi_name,
                thalm_colorsl[roi_pos, 0],
                thalm_colorsl[roi_pos, 1],
                thalm_colorsl[roi_pos, 2],
                0,
            )
        )
    nright = roi_pos + 1
...
class CorticalParcellation:
    ...
    def __init__(
        self,
        subjid: str = "fsaverage",
        hemi: str = "lh",
        subjects_dir: str = None,
        annot: str = "aparc",
        surface: str = "white",
        cortex_label: str = "cortex",
    ):
        """
        ...
        if os.path.isfile(annot):
            ...
            annot2load = os.path.join(
                subjects_dir, subjid, "label", hemi + "." + annot + ".annot"
            )
        ...
        if os.path.isfile(cortex_label):
            ...
            cortex_label2load = os.path.join(
                subjects_dir, subjid, "label", hemi + "." + cortex_label + ".label"
            )
    ...
    def load_annotation(self):
        ...
        if os.path.isfile(annot2load):
            ...
            raise ValueError(
                "Annotation file not found. Annotation, surface and cortex label files are mandatory to correct the parcellation."
            )
...
def _build_parcellation(fssubj_dir, subjid, growwm, out_dir, bool_mixwm, bool_rmwm):
    ...
    cort_dict = {
        ...
        "Type": "annot",
        "Name": [
            "atlas-laus2018_desc-scale1",
            "atlas-laus2018_desc-scale2",
            "atlas-laus2018_desc-scale3",
            "atlas-laus2018_desc-scale4",
            "atlas-laus2018_desc-scale5",
        ],
    ...
    if not os.path.isdir(fs_indivdir):
        ...
        surfatlas_dir = os.path.join(
            out_dir, cort_dict["OutSurfLocation"], subj_id, ses_id, "anat"
        )
        volatlas_dir = os.path.join(
            out_dir, cort_dict["OutVolLocation"], subj_id, ses_id, "anat"
        )
    ...
    wm_codes = np.concatenate(
        (wm_codesl.astype(int), wm_codesr.astype(int), cc_codes.astype(int))
    )
    ...
    subc_namesl = [
        s.replace("brain-stem", "bstem-mid-brainstem").lower() for s in subc_namesl
    ]
    ...
    if not os.path.isfile(os.path.join(fs_indivdir, "mri", "aparc+aseg.mgz")):
        print(
            f"FreeSurfer outputs are not complete please run freesurfer for subject {subjid}."
        )

    else:
        # Finding the cortical parcellations
        out_sparc = glob(
            os.path.join(surfatlas_dir, subjid + "*" + atlas_str + "*.annot")
        )

        if len(out_sparc) != len(atlas_names) * 2:
            print(
                f"The selected cortical parcellation ({cort_dict['Atlas']}) is not available for subject {subjid}"
            )
            ...
            for atlas in atlas_names:
                ...
                # 1. Left Hemisphere
                ind_annot = os.path.join(
                    fssubj_dir, subjid, "label", "lh." + atlas + ".annot"
                )  # Annot in individual space (freesurfer subject's directory)
                out_annot = os.path.join(
                    surfatlas_dir,
                    subjid + "_hemi-L_space-orig_" + atlas + "_dparc.annot",
                )
                if not os.path.isfile(out_annot):
                    if atlas_type == "annot":
                        fs_annot = os.path.join(
                            atlas_dir, "lh." + atlas + ".annot"
                        )  # Annot in fsaverage space (Atlas folder)
                        out_annot = _launch_annot2ind(
                            fs_annot, ind_annot, "lh", surfatlas_dir, subjid, atlas
                        )

                    elif atlas_type == "gcs":
                        fs_gcs = os.path.join(
                            atlas_dir, "lh." + atlas + ".gcs"
                        )  # GCS in fsaverage space (Atlas folder)
                        out_annot = _launch_gcs2ind(
                            fssubj_dir,
                            fs_gcs,
                            ind_annot,
                            "lh",
                ...
                out_sparc.append(
                    out_annot
                )  # Annot in individual space (Atlases subject's directory)

                # 2. Right Hemisphere
                ind_annot = os.path.join(
                    fssubj_dir, subjid, "label", "rh." + atlas + ".annot"
                )  # Annot in individual space (freesurfer subject's directory)
                out_annot = os.path.join(
                    surfatlas_dir,
                    subjid + "_hemi-R_space-orig_" + atlas + "_dparc.annot",
                )
                if not os.path.isfile(out_annot):
                    if atlas_type == "annot":
                        fs_annot = os.path.join(
                            atlas_dir, "rh." + atlas + ".annot"
                        )  # Annot in fsaverage space (Atlas folder)
                        out_annot = _launch_annot2ind(
                            fs_annot, ind_annot, "rh", surfatlas_dir, subjid, atlas
                        )

                    elif atlas_type == "gcs":
                        fs_gcs = os.path.join(
                            atlas_dir, "rh." + atlas + ".gcs"
                        )  # GCS in fsaverage space (Atlas folder)
                        out_annot = _launch_gcs2ind(
                            fssubj_dir,
                            fs_gcs,
        ...
        rh_cparc = [
            s for s in out_sparc if "hemi-R" in s
        ]  # Right cortical parcellation
        rh_cparc.sort()
        ...
        for g in growwm:
            tmp = glob(
                os.path.join(
                    volatlas_dir, subjid + "*" + atlas_str + "*grow" + g + "*.nii.gz"
                )
            )  # Cortical surface parcellation (.annot, .gii)
        ...
        if len(vol_cparc) != len(atlas_names) * len(growwm):
            vol_cparc = []
            for atlas in atlas_names:
                out_vol = _launch_surf2vol(
                    fssubj_dir, volatlas_dir, subjid, atlas, growwm
                )
        ...
        if len(rh_cparc) != len(
            lh_cparc
        ):  # Verifying the same number of parcellations for both hemispheres
            print(
        ...
        for (
            ...
            rh_cparc
        ):  # Verifying the existence of all surface-based cortical parcellations (Right)
            temp_file = Path(f)
        ...
        for (
            ...
            lh_cparc
        ):  # Verifying the existence of all surface-based cortical parcellations (Left)
            temp_file = Path(f)
            if not temp_file.is_file():
                print("Error: Some surface-based cortical parcellations are missing.\n")
                sys.exit(1)
        ...
        if temp_file.is_file():
            ...
            print(
                "Error: Cannot create the parcellation because there are missing files.\n"
            )
            sys.exit(1)
        ...
        outparc_lh = np.zeros(
            (dim[0], dim[1], dim[2]), dtype="int16"
        )  # Temporal parcellation for the left hemisphere
        outparc_rh = np.zeros(
            (dim[0], dim[1], dim[2]), dtype="int16"
        )  # Temporal parcellation for the right hemisphere

        ##### ========== Selecting Thalamic parcellation ============== #####

        # Thalamic parcellation based on Najdenovska et al, 2018
        vol_tparc = os.path.join(
            volthal_dir, subjid + "_space-orig_desc-MIALThalamicParc_dseg.nii.gz"
        )
        ...
        vol_tparc = _compute_abased_thal_parc(
            t1nii, vol_tparc, out_dir, subjid, aseg_nii, atlas_str
        )
        ...
        # Right Hemisphere
        outparc_rh, st_lengtrh = _search_in_atlas(
            temp_iparc, thalm_codesr, outparc_rh, 0
        )

        # Left Hemisphere
        outparc_lh, st_lengtlh = _search_in_atlas(
            temp_iparc, thalm_codesl, outparc_lh, 0
        )
        ...
        # Right Hemisphere
        outparc_rh, st_lengtrh = _search_in_atlas(
            aseg_parc, subc_codesr, outparc_rh, st_lengtrh
        )

        # Left Hemisphere
        outparc_lh, st_lengtlh = _search_in_atlas(
            aseg_parc, subc_codesl, outparc_lh, st_lengtlh
        )
        ...
        for i in np.arange(0, len(rh_cparc)):
            ...
            nctx_rh = len(
                rh_stnames
            )  # Number of cortical regions in the right hemisphere
            nctx_lh = len(
                lh_stnames
            )  # Number of cortical regions in the left hemisphere
            nroi_right = (
                nctx_rh + st_lengtrh
            )  # Number of regions in the right hemisphere
            ...
            for roi_pos, roi_name in enumerate(lh_stnames):
                ...
                wm_luttable.append(
                    "{:<4} {:<40} {:>3} {:>3} {:>3} {:>3}".format(
                        nroi_right + roi_pos + 3001,
                        temp_name,
                        255 - lh_colors[roi_pos, 0],
                        255 - lh_colors[roi_pos, 1],
                        255 - lh_colors[roi_pos, 2],
                        0,
                    )
                )
            ...
            for gparc in grow_parcs:
                ...
                templist = fname.split("_")
                tempVar = [
                    s for s in templist if "desc-" in s
                ]  # Left cortical parcellation
                ...
                outparcFilename = os.path.join(
                    volatlas_dir, "_".join(base_id) + "_dseg.nii.gz"
                )
                ...
                if not os.path.isfile(outparcFilename):
                    ...
                    out_atlas[ind[0], ind[1], ind[2]] = (
                        temp_iparc[ind[0], ind[1], ind[2]]
                        - np.ones((len(ind[0]),)) * 2000
                    )

                    # Adding the rest of the regions (Right Hemisphere)
                    ind = np.where(outparc_rh > 0)
                    out_atlas[ind[0], ind[1], ind[2]] = (
                        outparc_rh[ind[0], ind[1], ind[2]]
                        + np.ones((len(ind[0]),)) * nctx_rh
                    )
                    ...
                    out_atlas[ind[0], ind[1], ind[2]] = (
                        temp_iparc[ind[0], ind[1], ind[2]]
                        - np.ones((len(ind[0]),)) * 1000
                        + np.ones((len(ind[0]),)) * nroi_right
                    )

                    # Adding the rest of the regions (Left Hemisphere + Brainstem)
                    ind = np.where(outparc_lh > 0)
                    out_atlas[ind[0], ind[1], ind[2]] = (
                        outparc_lh[ind[0], ind[1], ind[2]]
                        + np.ones((len(ind[0]),)) * nctx_lh
                        + np.ones((len(ind[0]),)) * nroi_right
                    )

                    # Adding global white matter
                    bool_ind = np.isin(temp_iparc, wm_codes)
                    ...
                    # Saving the colorLUT
                    colorlutFilename = os.path.join(
                        volatlas_dir, "_".join(base_id) + "_dseg.lut"
                    )
                    ...
                    st_codes_lut, st_names_lut, st_colors_lut = read_fscolorlut(
                        colorlutFilename
                    )

                    # Saving the TSV
                    tsvFilename = os.path.join(
                        volatlas_dir, "_".join(base_id) + "_dseg.tsv"
                    )
                    _parc_tsv_table(
                        st_codes_lut, st_names_lut, st_colors_lut, tsvFilename
                    )
...
def main():
    ...
    if nthreads == 1:
        for i, subjid in enumerate(subjids):
            ...
            _build_parcellation(
                fssubj_dir, subjid, growwm, out_dir, bool_mixwm, bool_rmwm
            )
    else:
        start_time = time.perf_counter()

        ncores = os.cpu_count() or 1  # fallback if cpu_count() returns None

        # Reserve cores for the OS/other processes, but never go below 1
        if nthreads > ncores:
            nthreads = ncores
        reserved = max(1, ncores // 8)  # reserve ~12% of cores for the system
        nthreads = max(1, nthreads - reserved)

        # No point launching more workers than there are subjects
        nworkers = min(nthreads, nsubj)
        ...
        print(f"Finished in {end_time - start_time:.2f} seconds (s)...")
